...
export default function Blackjack() {
  ...
  useEffect((joueur1) => {
    ...
    function showDice(nb) {
      ...
      img.src =
        "https://github.com/alexis-feron/big-brain-games/blob/main/public/" +
        nb +
        ".png?raw=true";
      img.height = 150;
      div.appendChild(img);
    ...
    function waitForButtonClick() {
      return new Promise((resolve) => {
        ...
        btn4.addEventListener("click", () =>
          handleButtonClick("Ne pas lancer")
        );
      });
    }
      ...
      while (jetons_p1 > 0 && jetons_ordi > 0) {
        ...
        for (let i = 0; i < 3; i++) {
          ...
          if (selectedValue != "Ne pas lancer") {
            ...
            log(
              `${joueur1} a ` + score_p1 + " point" + (score_p1 > 1 ? "s" : "")
            );
            log("<br/>");
          } else {
        ...
        if (depassement_ordi && depassement_p1) {
          ...
          log(
            "EGALITE (" +
              ...
              "/" +
              score_ordi +
              "), ils déposent chacun 1 jeton dans la cagnotte"
          );
          log("<br/>");
          cagnotte += 2;
          ...
          log(
            "EGALITE (" +
              ...
              "/" +
              score_ordi +
              "), les jetons de la cagnotte restent en jeu"
          );
          if (score_p1 === 21) {
            blackjack += 1;
          }
    ...
    async function addWin(
      ...
      if (userConnected) {
        const resp = await userConnected.addBlackJackStats(
          userWon,
          diceThrown,
          blackjack,
          score,
      ...
      <main id="content" className={styles.content}>
        <div id="main" className={styles.main}>
          ...
          <div id="jetons" className={styles.jetons}>
            ...
            <span id="jetonsIa"></span> /<span id="jetonsJoueur"></span>
            <span className={styles.name} id="pseudo1">
              {joueur1}
            </span>
          </div>
          <div id="board" className={styles.board}>
            ...
            <div id="score" className={styles.score}>
              <h4>Score</h4>
              <span className={styles.name}>Ordinateur</span>
              <span id="scoreIa"></span> /<span id="scoreJoueur"></span>
              <span className={styles.name} id="pseudo2">
                {joueur1}
              </span>
            </div>
            ...
            <p id="goal" className={styles.goal}>
              {" "}
              Objectif : Avoir un plus grand score que son adversaire sans
              dépasser 21
            </p>
          </div>
        </div>
        <div id="log" className={styles.log}>
          <h3 className={styles.logTitle}>LOGS</h3>
          <hr />
          <div id="logMes" className={styles.logMes}></div>
          <button id="playButton" className={styles.playButton}>
            {" "}
            <FontAwesomeIcon
              className={styles.arrowIcon}
              icon={faCircleArrowRight}
            />{" "}
            <span className={styles.spanButton}>Relancer la partie</span>
          </button>
        </div>
      </main>
    </div>
