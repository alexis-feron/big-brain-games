...
export default function Home() {
  ...
  useEffect(() => {
    document
      .querySelector("#bt_remove")
      .addEventListener("click", player_remove);
    document
    ...
    function play() {
      ...
      if (document.querySelector("#game_board img") === null) {
        for (let i = 0; i < allumette_restante; i++) {
          ...
          img.src =
            "https://github.com/alexis-feron/big-brain-games/blob/main/public/allumette.png?raw=true";
          img.height = 150;
          img.style.transform =
            "rotate(" +
      ...
      document.querySelector("#name_player_get").innerHTML =
        "C'est à " + nom_joueur + " de jouer !";
    }

    function click_allumette() {
      nb_allumette_prise =
        document.querySelectorAll("#game_board .check").length;
          ...
          <div id="board" className={styles.board}>
            ...
            <div id="game_validation" className={styles.game_validation}>
              <input
                type="button"
                id="bt_remove"
                className={styles.bt_remove}
                value="Valider"
              />
              <p id="goal" className={styles.goal}>
                {" "}
                Objectif : ne pas avoir la dernière allumette
              </p>
            </div>
          </div>
        </div>
